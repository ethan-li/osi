...
class TestDistributionScripts(unittest.TestCase):
    ...
    def test_build_scripts_directory_exists(self):
        ...
        self.assertTrue(build_scripts_dir.exists(), "build_scripts directory should exist")
        self.assertTrue(build_scripts_dir.is_dir(), "build_scripts should be a directory")
...
class TestDockerConfiguration(unittest.TestCase):
    ...
    def test_docker_compose_exists(self):
        ...
        if docker_compose.exists():
            self.assertTrue(docker_compose.is_file(), "docker-compose.yml should be a file")
    ...
    def test_platform_launchers_exist(self):
        ...
        for launcher, description in launchers.items():
            ...
            self.assertTrue(
                ...
                f"{description} ({launcher}) should exist"
            )
        ...
        if requirements.exists():
            ...
            for line in lines:
                if not line.startswith('#'):  # Skip comments
                    self.assertGreater(
                        len(line), 0, 
                        "Each requirement line should not be empty"
                    )
