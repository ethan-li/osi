...
class DependencyResolver:
    ...
    def get_missing_dependencies(self, tool_name: str) -> List[str]:
        ...
        try:
            ...
            self.logger.error(f"Failed to check missing dependencies for {tool_name}: {e}")
    ...
    def install_missing_dependencies(self, tool_name: str) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Installing missing dependencies for {tool_name}: {missing_deps}")
            ...
            self.logger.error(f"Failed to install missing dependencies for {tool_name}: {e}")
            return False
        ...
        try:
            ...
            for package_name, requirements in package_requirements.items():
                if len(requirements) > 1:
                    ...
                    version_specs = [req['version_spec'] for req in requirements if req['version_spec']]
                    
                    if len(set(version_specs)) > 1:
                        # Different version specifications - potential conflict
                        conflicts[package_name] = [req['requirement'] for req in requirements]
    ...
    def ensure_tool_dependencies(self, tool_name: str) -> bool:
        ...
        try:
            ...
            if not self.env_manager.validate_environment(tool_name):
                self.logger.warning(f"Environment for {tool_name} is invalid, recreating")
                if not self.env_manager.create_environment(tool_name):
                    return False
